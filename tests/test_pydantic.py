...
def test_zero_quantity_rejected():
    with pytest.raises(ValidationError):
        StockUpdateInput(item_id="A101", quantity=0.0, operation="add")
...
def test_invalid_item_id_format():
    ...
    with pytest.raises(ValidationError):
        StockUpdateInput(item_id="INVALID", quantity=1.0, operation="add")


def test_invalid_item_id_lowercase():
    with pytest.raises(ValidationError):
        StockUpdateInput(item_id="a101", quantity=1.0, operation="add")


def test_invalid_operation():
    with pytest.raises(ValidationError):
        StockUpdateInput(item_id="A101", quantity=1.0, operation="order")


def test_valid_input_add():
    inp = StockUpdateInput(item_id="A101", quantity=3.0, operation="add")
    assert inp.quantity == 3.0
    assert inp.operation == "add"


def test_valid_input_consume():
    inp = StockUpdateInput(item_id="D500", quantity=1.5, operation="consume")
